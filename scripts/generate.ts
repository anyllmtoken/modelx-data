/**
 * Codegen: reads providers-us/ → writes src/data-us.ts
 *
 * Usage:
...
const DATA_OUTPUT_US = path.join(ROOT, "src", "data-us.ts");
...
{
  const providersDir = path.join(ROOT, "providers-us");
  if (!fs.existsSync(providersDir)) {
    console.warn("Directory not found: providers-us/ — skipping");
    process.exit(0);
  }
  ...
  fs.writeFileSync(DATA_OUTPUT_US, lines.join("\n"), "utf-8");

  const totalModels = providers.reduce((sum, p) => sum + p.models.length, 0);
  console.log(
    `Generated data-us.ts: ${providers.length} providers, ${totalModels} models`,
  );
}
