 ...
 * Scans both providers-cn/ and providers-us/ directories.
 * Writes results to packages/data/changes.jsonl in the format consumed by the web app.
 *
 * Usage:
...
function getChangedFiles(ref: string): {
  ...
  for (const dir of PROVIDERS_DIRS) {
    let output: string;
    try {
      output = git(`diff --name-status ${ref} -- ${dir}`);
    } catch {
      continue;
    }
...
function main() {
  ...
  console.log(`Scanning directories: ${PROVIDERS_DIRS.join(", ")}`);
